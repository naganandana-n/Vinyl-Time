...
# 🔹 Step 6: Detect Circular Shapes from the Filtered Edges
circle_contours = []
for cnt in top_contours:
    ...
    if 0.75 < aspect_ratio < 1.25 and circularity > 0.7:  # Tune values as needed
        circle_contours.append(cnt)

# 🔹 Step 7: Draw Only the Detected Circular Figures
...
cv2.imshow("Detected Circular Figures (Contour-Based)", output)
cv2.waitKey(0)
